...
- Command: 'add'
  ...
  Usage: "link <word/phrase> to <http://...>"
...
- Command: 'find'
  Regex: '(?i:look ([-\w ,''!."+=?&@#()/]+) up)'
  Contexts: [ "item:it" ]
  Usage: "look <keyword/phrase> up"
  Summary: "Alternate phrasing for finding saved links."
  Examples:
  - "(alias) look gopherbot up"
ReplyMatchers:
- Regex: '([-\w ,''!."+=?&@#()/]+)'
  Label: "lookup"
`)
