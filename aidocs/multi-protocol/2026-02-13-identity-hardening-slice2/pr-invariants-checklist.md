...
- PR slice: `identity-hardening-slice2`
- Linked impact report: `aidocs/multi-protocol/2026-02-13-identity-hardening-slice2/impact-surface-report.md`
...
- Transport-specific internal IDs map explicitly to shared username: yes (`UserMap`, plus legacy `UserRoster.UserID` bridge)
...
- Focused tests added/updated: yes (`bot/conf_identity_test.go`, `bot/handler_identity_test.go`)
...
- Residual risks:
  - mixed legacy + new mapping data can be confusing during migration until legacy IDs are removed.
- Follow-up items:
  - consider protocol-scoping thread subscription keys for full cross-protocol isolation.
