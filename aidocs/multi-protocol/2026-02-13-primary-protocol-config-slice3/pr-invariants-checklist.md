...
- PR slice: `primary-protocol-config-slice3`
- Linked impact report: `aidocs/multi-protocol/2026-02-13-primary-protocol-config-slice3/impact-surface-report.md`
...
- Focused tests added/updated: yes (`bot/conf_protocol_test.go`)
...
- Residual risks:
  - default/sample configs that still include protocol config in `robot.yaml` will emit compatibility warnings until migrated.
- Follow-up items:
  - migrate templates to preferred primary config layout (no embedded `ProtocolConfig` in `robot.yaml`).
