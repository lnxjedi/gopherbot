...
- PR slice: `identity-usermap-slice1`
- Linked impact report: `aidocs/multi-protocol/2026-02-13-identity-usermap-slice1/impact-surface-report.md`
...
- Transport-specific internal IDs map explicitly to shared username: yes (`UserMap` + compatibility mapping)
...
- Focused tests added/updated: yes (`bot/conf_identity_test.go`)
...
- Other affected docs updated: yes (`aidocs/STARTUP_FLOW.md`)

## Sign-Off

- Residual risks:
  - Global fallback maps still exist for compatibility and should be removed in a follow-up hardening slice.
- Follow-up items:
  - tighten `IgnoreUnlistedUsers` to require both directory + protocol map.
  - remove legacy `UserRoster.UserID` support after migration window.
