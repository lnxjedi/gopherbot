...
func (tc *TestConnector) SendProtocolChannelThreadMessage(ch, thr, mesg string, f robot.MessageFormat, dummyMsgObject interface{}) (ret robot.RetVal) {
...
func (tc *TestConnector) SendProtocolUserChannelThreadMessage(uid, uname, ch, thr, mesg string, f robot.MessageFormat, dummyMsgObject interface{}) (ret robot.RetVal) {
	channel := tc.getChannel(ch)
	msg := &BotMessage{
		User:    uname,
		Channel: channel,
...
func (tc *TestConnector) SendProtocolUserMessage(u string, mesg string, f robot.MessageFormat, dummyMsgObject interface{}) (ret robot.RetVal) {
	var user *testUser
	var exists bool
	if user, exists = tc.getUserInfo(u); !exists {
		return robot.UserNotFound
