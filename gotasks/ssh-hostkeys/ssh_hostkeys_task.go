...
func sshHostKeysTask(r robot.Robot, args ...string) (retval robot.TaskRetVal) {
	...
	case "loadhostkeys":
		...
		if insecureCloneStr == "true" {
			insecureClone = true
		}

		knownHostsPath, handle, err := sshhostkeys.LoadHostKeys(repoURL, insecureClone)
		if err != nil {
			r.Log(robot.Error, "failed to load host keys: "+err.Error())
			return robot.Fail
		}

		if knownHostsPath == "" && insecureClone {
			// Proceed without host keys
			sshOptions, err := buildSSHOptionsInsecure(r)
			if err != nil {
				r.Log(robot.Error, "failed to build SSH options: "+err.Error())
				return robot.Fail
			}
			r.Log(robot.Warn, "Proceeding with insecure SSH clone without host key verification")
			r.SetParameter("SSH_OPTIONS", sshOptions)
			return robot.Normal
		}
...
// Helper function to build SSH_OPTIONS for insecure clone
func buildSSHOptionsInsecure(r robot.Robot) (string, error) {
	sshOptions := "-o PasswordAuthentication=no -o StrictHostKeyChecking=no"
