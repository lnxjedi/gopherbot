...
configuration in the `testcfg` directory. You'll want to copy the directory and modify
it for your own use (mainly configuring the plugins you're developing), but it can be used
by using the `-c <configdir>` option:
```
[./gopherbot]$ go build
[./gopherbot]$ ./gopherbot -l /tmp/bot.log -c testcfg/
...
My local configuration directory is: testcfg/
