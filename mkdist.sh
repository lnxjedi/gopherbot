...
do
	...
	then
		...
		zip -r $OUTFILE gopherbot.exe LICENSE README.md brain/ conf/ doc/ testcfg/ lib/ licenses/ misc/ plugins/ --exclude *.swp
	else
		GOOS=$BUILDOS go build
		echo "Creating $OUTFILE"
		zip -r $OUTFILE gopherbot LICENSE README.md brain/ conf/ doc/ testcfg/ lib/ licenses/ misc/ plugins/ --exclude *.swp
	fi
