...
do
	...
	then
		...
		zip -r $OUTFILE gopherbot.exe LICENSE README.md brain/ conf/ doc/ cfg/ lib/ licenses/ misc/ plugins/ jobs/ tasks/ --exclude *.swp --exclude conf/*.yaml --exclude conf/*/*.yaml
	else
		GOOS=$BUILDOS go build
		echo "Creating $OUTFILE"
		zip -r $OUTFILE gopherbot LICENSE README.md brain/ conf/ doc/ cfg/ lib/ licenses/ misc/ plugins/ jobs/ tasks/ --exclude *.swp --exclude conf/*.yaml --exclude conf/*/*.yaml
	fi
