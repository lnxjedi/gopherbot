...
func TestStartupSSHHint(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			protocol: "ssh",
			admin:    []string{"david"},
			wantHint: "Running in DEMO mode; in another terminal window, connect as david with 'bot-ssh -l david'",
		},
		{
			...
			protocol: "ssh",
			admin:    []string{"   "},
			wantHint: "Running in DEMO mode; in another terminal window, connect as alice with 'bot-ssh -l alice'",
		},
		{
			...
			protocol: "ssh",
			admin:    []string{"alice"},
			wantHint: "Default robot running in test-dev mode with ssh-connector; connect with e.g. 'bot-ssh -l alice'",
		},
		{
