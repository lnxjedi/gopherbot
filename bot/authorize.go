...
func (c *botContext) checkAuthorization(t interface{}, command string, args ...string) (retval robot.TaskRetVal) {
	...
	if authPlug != nil {
		...
		if authRet == robot.Fail {
			Log(robot.Audit, "Authorization FAILED by authorizer '%s' for user '%s' calling command '%s' for task '%s' in channel '%s'; AuthRequire: '%s'", authPlug.name, c.User, command, task.name, c.Channel, task.AuthRequire)
			r.Say("Sorry, you're not authorized for that command")
			emit(AuthRanFail)
			return robot.Fail
		}
		if authRet == robot.MechanismFail {
			Log(robot.Audit, "Auth plugin '%s' mechanism failure while authenticating user '%s' calling command '%s' for task '%s' in channel '%s'; AuthRequire: '%s'", authPlug.name, c.User, command, task.name, c.Channel, task.AuthRequire)
			r.Say(technicalAuthError)
			emit(AuthRanMechanismFailed)
			return robot.MechanismFail
		}
		if authRet == robot.Normal {
			Log(robot.Audit, "Auth plugin '%s' returned 'Normal' (%d) instead of 'Success' (%d), failing auth in '%s' calling command '%s' for task '%s' in channel '%s'; AuthRequire: '%s'", authPlug.name, robot.Normal, robot.Success, c.User, command, task.name, c.Channel, task.AuthRequire)
			r.Say(technicalAuthError)
			emit(AuthRanFailNormal)
			return robot.MechanismFail
		}
		Log(robot.Audit, "Auth plugin '%s' exit code %s, failing auth while authenticating user '%s' calling command '%s' for task '%s' in channel '%s'; AuthRequire: '%s'", authPlug.name, authRet, c.User, command, task.name, c.Channel, task.AuthRequire)
		r.Say(technicalAuthError)
