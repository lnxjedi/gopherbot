...
func (bot *Robot) checkTaskMatchersAndRun(matcherType int) (messageMatched bool) {
	...
	for _, t := range *bot.tasks.t {
		...
		case plugCommands:
			...
			if len(plugin.CommandMatchers) == 0 {
				bot.debug(task.taskID, fmt.Sprintf("Plugin has no command matchers, skipping command check"), false)
				continue
			}
			isPlugin = true
		...
		case plugMessages:
			...
			if len(plugin.MessageMatchers) == 0 {
				bot.debug(task.taskID, fmt.Sprintf("Plugin has no message matchers, skipping message check"), true)
				continue
			}
			isPlugin = true
		...
		bot.debug(task.taskID, fmt.Sprintf("Checking %d %s matchers against message: '%s'", len(matchers), ctype, bot.msg), verboseOnly)
		for _, matcher := range matchers {
			Log(Trace, fmt.Sprintf("Checking '%s' against '%s'", bot.msg, matcher.Regex))
			matches := matcher.re.FindAllStringSubmatch(bot.msg, -1)
			matched := false
			if matches != nil {
				bot.debug(task.taskID, fmt.Sprintf("Matched %s regex '%s', command: %s", ctype, matcher.Regex, matcher.Command), false)
				matched = true
				...
				bot.debug(task.taskID, fmt.Sprintf("Not matched: %s", matcher.Regex), verboseOnly)
			}
			if matched {
				if messageMatched {
	...
	if messageMatched {
		task, plugin, job := getTask(runTask)
		isPlugin := plugin != nil
		bot.messageHeard()
