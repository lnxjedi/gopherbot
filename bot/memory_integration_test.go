...
func TestMemory(t *testing.T) {
	...
	tests := []testItem{
		...
		{carol, general, "Bender, look up nerds", []testc.TestMessage{{null, general, `(?s:Here's what I have .*Nerds.*)`}}, []Event{CommandPluginRan, GoPluginRan}, 0},
		{alice, general, ";link tuna casserole to https://www.allrecipes.com/recipe/17219/best-tuna-casserole/", []testc.TestMessage{{null, general, `Link added`}}, []Event{CommandPluginRan, GoPluginRan}, 0},
		{alice, general, ";add it to the dinner meals list", []testc.TestMessage{{alice, general, `I don't have a .*`}}, []Event{CommandPluginRan, GoPluginRan}, 0},
		{alice, general, "yes", []testc.TestMessage{{null, general, `Ok, .*`}}, []Event{}, 0},
		{alice, general, "Bender, look it up", []testc.TestMessage{{null, general, `(?s:Here's what I have .*best.*)`}}, []Event{CommandPluginRan, GoPluginRan}, 0},
		{alice, general, "add hamburgers to the list, bender", []testc.TestMessage{{null, general, `Ok, I added hamburgers to the dinner meals list`}}, []Event{CommandPluginRan, GoPluginRan}, 0},
	}
