...
/**
 * Retrieves the configured OAuth2 provider metadata for a provider key.
 *
 * Retrieves a bearer token for a configured OAuth2 provider and user.
 *
 * @param {string} provider - Provider key from robot.yaml.
 * @param {string} user - Canonical Gopherbot username.
 * @returns {{ token: string, retVal: number }}
 */
Robot.prototype.GetOAuth2Token = function (provider, user) {
  return this.gbot.GetOAuth2Token(provider, user);
};

/**
 * Stores a linked OAuth2 token set for a provider/user pair.
 *
 * @param {object} link - Link metadata including provider, user, accessToken, and optional refresh token fields.
 * @returns {number}
 */
Robot.prototype.LinkOAuth2User = function (link) {
  return this.gbot.LinkOAuth2User(link);
};
 ...
 * @returns {number}
 */
Robot.prototype.UnlinkOAuth2User = function (provider, user) {
  return this.gbot.UnlinkOAuth2User(provider, user);
};
