...
/**
 * Sends a message to a specific protocol with user/channel routing semantics.
 *
 * Semantics:
 * - user + no channel => DM
 * - channel + no user => channel message
 * - user + channel => user mention in channel
 *
 * @param {string} protocol - Protocol name ("ssh", "slack", etc.)
 * @param {string} user - Optional user target
 * @param {string} channel - Optional channel target
 * @param {string} message - The message to send
 * @param {fmt} [format] - Optional format (fmt.*) for the message
 * @returns {number} - The retVal return code (ret.*)
 */
Robot.prototype.SendProtocolUserChannelMessage = function (protocol, user, channel, message, format) {
  return this.gbot.SendProtocolUserChannelMessage(protocol, user, channel, message, format);
};
...
/**
 * Simple message send - if the original incoming message was in a channel or DM,
 * the message sent is also. If the original message was in a thread, Say will
 * send the message to that thread. Basically: emit a response to something a user said.
 *
 * @param {string} message - The message to send
 * @param {fmt} [format] - Optional format (fmt.*) for the message
 * @returns {number} - The retVal return code (ret.*)
...
/**
 * Like Say, but will create a thread if the incoming message was in a channel.
 *
 * @param {string} message - The message to send
 * @param {fmt} [format] - Optional format (fmt.*) for the message
 * @returns {number} - The retVal return code (ret.*)
 *
 * @example
 * bot.SayThread("Hello, new thread!");
 */
...
/**
 * Replies to a message - like Say but mentioning the user.
 *
...
/**
 * Replies to a message in a thread - like SayThread but mentioning the user.
 *
 * @param {string} message - The reply message.
 * @param {fmt} [format] - Optional format for the reply.
 * @returns {number} - The retVal return code (ret.*)
 *
 * @example
 * bot.ReplyThread("Let's start a thread!", fmt.Fixed);
 * -> (new thread) "@david Let's start a new thread!"
 */
...
// The robot returned by Direct sends Say and Reply messages to
// the user in a DM.
...
// The robot returned by Fixed always sends messages in fixed-
// width font.
...
// The robot returned by Threaded sets the threaded flag true,
// so a normal Say or Reply will be treated like SayThread or
// ReplyThread; simplifies plugins that generate multiple
...
// The robot returned by MessageFormat always sends messages
// in the given format.
...
/**
 * Prompts for a reply matching a regex, always in a thread
 *
 * @param {string} regex_id - The regex identifier
 * @param {string} prompt - The prompt message
 * @param {fmt} [format] - Optional format (fmt.*) for the prompt
...
/**
 * Prompts a specific user via DM for a reply matching a regex
 *
 * @param {string} regex_id - The regex identifier
 * @param {string} user - The user identifier
 * @param {string} prompt - The prompt message
 ...
 *
 * @example
 * const result = bot.PromptUserChannelForReply("YesNo", "archie", "#devops", "User 'ritchie' requested to deploy software, do you approve?");
 */
 ...
 * (honestly I can't think of any good use-cases for this, but *shrug*)
 *
 * @param {string} regex_id - The regex identifier
 * @param {string} user - The user identifier
 * @param {string} channel - The channel identifier
...
/**
 * Short-term memories are contextual memories that expire after a time - generally specific to a
 * given user in a channel. Most often these are used so the robot can "remember" that e.g. user
 * "david" has been working with e.g. the server "web2.mycompany.com".
 * 
 * When the memory is "shared", the context is the channel and potentially a thread in the channel,
 * but not specific to a given user. The AI plugin uses this for multi-user AI discussions.
 */
 ...
 * @param {boolean} [shared] - Whether the memory is shared with all users in the channel.
 * @returns {undefined} - This method returns no value (undefined)
 *
 * @example
 * bot.Remember("currentTopic", "robot memories", true);
 */
...
/**
 * Remembers a key-value pair short-term memory, contextualized to the thread.
 *
 * @param {string} key - The key to remember.
 * @param {string} value - The value to associate with the key.
 ...
 * these contexts need to be referenced in the plugin configuration
 * to be useful.
 *
...
/**
 * Deletes a value from short-term memory in the current context.
 *
 * @param {string} key - The key to delete.
 * @param {boolean} [shared] - Whether to delete shared memory.
 * @returns {undefined} - This method returns no value (undefined)
 *
 * @example
 * bot.DeleteMemory("favoriteColor", true);
 */
Robot.prototype.DeleteMemory = function (key, shared) {
  return this.gbot.DeleteMemory(key, shared);
};
...
/**
 * Subscribe the current plugin invocation to future messages in this thread.
 *
 * @returns {boolean} - `true` on success.
 */
Robot.prototype.Subscribe = function () {
  return this.gbot.Subscribe();
};

/**
 * Remove a prior thread subscription for the current plugin invocation.
 *
 * @returns {boolean} - `true` on success.
 */
Robot.prototype.Unsubscribe = function () {
  return this.gbot.Unsubscribe();
};

/**
 * Exclusive implements a kind of advisory "mutex" - the first job, task or plugin to request e.g. "Exclusive("prodbuild", true)"
 * will get a "true" reply, and can proceed under the assumption no other running processes will be modifying
 * resources associated with "prodbuild". If "queue" is set, then the extension should exit (after potentially
 * sending a status update message), and the engine will re-start the task/plugin/job when the mutex is free.
 * 
 * If an extension gets a "false" reply and continues to futz around with the resource meant to be protected,
 * well - that's a Bad Idea (tm). "false" means another task in is messing with it.
 *
...
/**
 * Spawns a new job; the Gopherbot equivalent of "fork/exec" - the spawned job starts immediately and proceeds
 * in parallel to the current pipeline.
 *
...
/**
 * Adds a new task to the pipeline. After the current task finishes, the next task in the pipeline starts.
 *
...
/**
 * Adds a task to the final pipeline. After the main pipeline finishes, all the tasks in the final
 * pipeline are run in reverse order - these are meant to clean up resources from the primary
 * pipeline, and the reverse order is meant to deal with any dependencies, e.g.:
 * main: start_a, start_b_dependson_a
 * final: stop_b_dependedon_a, stop_a
 *
 * @param {string} name - The task name.
 * @param {...string} args - Arguments for the task.
 * @returns {number} - The retVal return code (ret.*)
...
/**
 * Adds a task to the fail pipeline, which runs if any task in the main pipeline fails.
 * Normally tells the user that something failed.
 *
 * @param {string} name - The task name.
 * @param {...string} args - Arguments for the task.
 * @returns {number} - The retVal return code (ret.*)
...
/**
 * Adds a new job to the pipeline. Unlike spawn, the job will run in the same thread
 * of execution as the current task. The added job can in turn create it's own pipeline
 * which will run to completion before the current pipeline resumes.
 *
 * @param {string} name - The job name.
 * @param {...string} args - Arguments for the job.
 * @returns {number} - The retVal return code (ret.*)
...
/**
 * Adds a new plugin command to the pipeline. Similar to adding a job, it starts a plugin
 * as if the user had typed a particular message"
 *
 * @param {string} pluginName - The name of the plugin.
 * @param {string} command - The command to send to the plugin as if the user had typed it.
 * @returns {number} - The retVal return code (ret.*)
 *
 * @example
...
/**
 * Adds a plugin command to the fail pipeline.
 *
 * @param {string} pluginName - The name of the plugin.
 * @param {string} command - The command to add.
 * @returns {number} - The retVal return code (ret.*)
...
/**
 * Long-term memories are kept indefinitely, and are meant to hold things like grocery
 * lists and state information for e.g. software builds and deployments. The only
 * context is the configured namespace, so that e.g. the "prod" namespace and "dev"
 * namespace (configured in robot.yaml) can both use e.g. a "deployment" memory, which
 * will be private to the namespace.
 */
/**
 * Checks out a datum from long-term memory. The current task gets a short-term lock on
 * the memory, and other requesters will block until the timeout expires; after the timeout
 * expires, updating the memory could fail if another task had requested the same memory.
 * For operations where you can't guarantee the memory is checked out and updated quickly,
 * use Exclusive instead.
 *
 * @param {string} key - The datum key.
 * @param {boolean} [rw=false] - Whether to request read-write access.
 * @returns {{ retVal: number, key: string, exists: boolean, datum: any, token: string }} 
...
Robot.prototype.CheckoutDatum = function (key, rw) {
  ...
  const memObj = this.gbot.CheckoutDatum(key, rw);
  memObj.key = key;
  return memObj;
};
 ...
 *   - `datum`: The updated JSON-compatible data to store. Cycles, sparse
 *     arrays, and values such as `undefined`, functions, symbols, `BigInt`,
...
/**
 * Checks in a previously checked-out datum to long-term memory. indicating the memory wasn't modified
 * and freeing it up for another requester.
 *
...
/**
 * Deletes a datum from long-term memory.
 *
 * @param {string} key - The datum key.
 * @returns {number} The return value constant.
 *
 * @example
 * const rv = bot.DeleteDatum("myKey");
 * bot.Say("DeleteDatum returned: " + ret.string(rv));
 */
Robot.prototype.DeleteDatum = function (key) {
  return this.gbot.DeleteDatum(key);
};

/**
 * Retrieves the current task configuration, set in the plugin or job yaml file.
 *
...
/**
 * Retrieves a user-linked identity credential for a configured provider and user.
 *
 * @param {string} provider - Provider key from robot.yaml.
 * @param {string} user - Canonical Gopherbot username.
 * @returns {{ credential: object | null, retVal: number }}
 */
Robot.prototype.GetIdentityCredential = function (provider, user) {
  return this.gbot.GetIdentityCredential(provider, user);
};

/**
 * Stores a linked OAuth2 identity for a provider/user pair.
 *
 * @param {object} link - Link metadata including provider, user, accessToken, and optional refresh token fields.
 * @returns {number}
 */
Robot.prototype.LinkOAuth2Identity = function (link) {
  return this.gbot.LinkOAuth2Identity(link);
};

/**
 * Encrypts plaintext for safe use in custom conf/variables Secrets entries.
 *
 * @param {string} plaintext
 * @returns {{ ciphertext: string, retVal: number }}
 */
Robot.prototype.EncryptSecret = function (plaintext) {
  const result = this.gbot.EncryptSecret(plaintext);
  return {
    ciphertext: String(result.ciphertext || ""),
    retVal: Number(result.retVal),
  };
};

/**
 * Removes a linked identity provider for a user.
 *
 * @param {string} provider - Provider key from robot.yaml.
 * @param {string} user - Canonical Gopherbot username.
 * @returns {number}
 */
Robot.prototype.UnlinkIdentity = function (provider, user) {
  return this.gbot.UnlinkIdentity(provider, user);
};
 ...
 * // NOTE: There's no retVal for Pause; it simply returns undefined
 */
...
/**
 * Elevates the current user's privileges - similar to "sudo", the user has to reply
 * with something like a 2fa token or password, proving the user's identity.
 *
 ...
 *
 * @param {number} level - The log level constant (matches `log.*`).
