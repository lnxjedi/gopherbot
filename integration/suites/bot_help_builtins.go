...
func init() {
	Register(Suite{
		...
		Cases: legacyCases([]testItem{
			...
			{aliceID, null, ";set log level fine", false, []TestMessage{{alice, null, "Invalid value 'fine' for 'level'; valid values: trace, debug, info, warn, error\\.", false}}, []bot.Event{bot.BotDirectMessage}, 0},
			{aliceID, null, ";set log lines to two", false, []TestMessage{{alice, null, "Invalid value 'two' for 'lines'; expected an integer\\.", false}}, []bot.Event{bot.BotDirectMessage}, 0},
			...
			{aliceID, general, ";list jobs", false, []TestMessage{{null, general, `(?s:(Here's a list of jobs for this channel:|I don't see any jobs configured for this channel))`, false}}, []bot.Event{bot.CommandTaskRan, bot.GoPluginRan}, 0},
			{aliceID, general, ";list all jobs", false, []TestMessage{{null, general, `(?s:(Here's a list of all the jobs I know about:|I dont' have any jobs configured))`, false}}, []bot.Event{bot.CommandTaskRan, bot.GoPluginRan}, 0},
