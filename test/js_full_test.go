...
func TestJSFull(t *testing.T) {
	...
	tests := []testItem{
		...
		{bobID, general, ";js-memory-check", false, []TestMessage{
			{null, general, "MEMORY CHECK: local=<empty> shared=<empty> ctx=<empty> thread=<empty> threadctx=<empty>", false}},
			[]Event{CommandTaskRan, ExternalTaskRan}, 0},
		{aliceID, general, ";js-memory-thread-delete", true, []TestMessage{
		...
		{aliceID, general, ";js-memory-thread-check", true, []TestMessage{
			...
			[]Event{CommandTaskRan, ExternalTaskRan}, 0},
		{aliceID, general, ";js-memory-datum-seed", false, []TestMessage{
			{null, general, "MEMORY DATUM SEED: update=Ok", false}},
			[]Event{CommandTaskRan, ExternalTaskRan}, 0},
		{aliceID, general, ";js-memory-datum-check", false, []TestMessage{
		...
		{aliceID, general, ";js-memory-datum-checkin", false, []TestMessage{
			...
			[]Event{CommandTaskRan, ExternalTaskRan}, 0},
		{aliceID, general, ";js-memory-datum-delete", false, []TestMessage{
		...
		{aliceID, general, ";js-identity", false, []TestMessage{
			...
			[]Event{CommandTaskRan, ExternalTaskRan}, 0},
		{aliceID, general, ";js-parameter-addtask", false, []TestMessage{
...
func TestJSFullSecurity(t *testing.T) {
	if !wantFull("js") {
		t.Skip("skipping JS full security test; set RUN_FULL=js (or RUN_JSFULL=1)")
	}
	done, conn := setup("test/jsfull", "/tmp/bottest.log", t)

	flow := []testItem{
		{aliceID, general, ";js-sec-open", false, []TestMessage{
			{null, general, "SECURITY CHECK: secopen", false}}, nil, 0},
		{bobID, general, ";js-sec-open", false, []TestMessage{
			{null, general, "SECURITY CHECK: secopen", false}}, nil, 0},
		{aliceID, general, ";js-sec-admincmd", false, []TestMessage{
			{null, general, "SECURITY CHECK: secadmincmd", false}}, nil, 0},
		{bobID, general, ";js-sec-admincmd", false, []TestMessage{
			{null, general, "Sorry, 'jssec/secadmincmd' is only available to bot administrators", false}}, nil, 0},
		{bobID, general, ";js-sec-authz", false, []TestMessage{
			{null, general, "SECURITY CHECK: secauthz", false}}, nil, 0},
		{davidID, general, ";js-sec-authz", false, []TestMessage{
			{null, general, "Sorry, you're not authorized for that command", false}}, nil, 0},
		{bobID, general, ";js-sec-authall", false, []TestMessage{
			{null, general, "SECURITY CHECK: secauthall", false}}, nil, 0},
		{davidID, general, ";js-sec-authall", false, []TestMessage{
			{null, general, "Sorry, you're not authorized for that command", false}}, nil, 0},
		{aliceID, general, ";js-sec-elevated", false, []TestMessage{
		...
		{aliceID, general, ";js-sec-immediate", false, []TestMessage{
			{alice, general, "This command requires immediate elevation.*TOTP code.*", false}}, nil, 150},
		{aliceID, general, "123456", false, []TestMessage{
			{null, general, "There were technical issues validating your code.*", false},
			{null, general, "Sorry, elevation failed due to a problem with the elevation service", false}}, nil, 0},
