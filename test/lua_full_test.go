...
func TestLuaFullSecurity(t *testing.T) {
	if !wantFull("lua") {
		t.Skip("skipping Lua full security test; set RUN_FULL=lua (or RUN_LUAFULL=1)")
	}
	done, conn := setup("test/luafull", "/tmp/bottest.log", t)

	flow := []testItem{
		{aliceID, general, ";lua-sec-open", false, []TestMessage{
			{null, general, "SECURITY CHECK: secopen", false}}, nil, 0},
		{bobID, general, ";lua-sec-open", false, []TestMessage{
			{null, general, "SECURITY CHECK: secopen", false}}, nil, 0},
		{aliceID, general, ";lua-sec-admincmd", false, []TestMessage{
			{null, general, "SECURITY CHECK: secadmincmd", false}}, nil, 0},
		{bobID, general, ";lua-sec-admincmd", false, []TestMessage{
			{null, general, "Sorry, 'luasec/secadmincmd' is only available to bot administrators", false}}, nil, 0},
		{bobID, general, ";lua-sec-authz", false, []TestMessage{
			{null, general, "SECURITY CHECK: secauthz", false}}, nil, 0},
		{davidID, general, ";lua-sec-authz", false, []TestMessage{
			{null, general, "Sorry, you're not authorized for that command", false}}, nil, 0},
		{bobID, general, ";lua-sec-authall", false, []TestMessage{
			{null, general, "SECURITY CHECK: secauthall", false}}, nil, 0},
		{davidID, general, ";lua-sec-authall", false, []TestMessage{
			{null, general, "Sorry, you're not authorized for that command", false}}, nil, 0},
		{aliceID, general, ";lua-sec-elevated", false, []TestMessage{
		...
		{aliceID, general, ";lua-sec-immediate", false, []TestMessage{
			{alice, general, "This command requires immediate elevation.*TOTP code.*", false}}, nil, 150},
		{aliceID, general, "123456", false, []TestMessage{
			{null, general, "There were technical issues validating your code.*", false},
			{null, general, "Sorry, elevation failed due to a problem with the elevation service", false}}, nil, 0},
